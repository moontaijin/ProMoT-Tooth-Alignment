...
from data.utils import get_files, walkFile, rotate_maxtrix, se3_log_map, quaternion_to_axis_angle
...
def train_data_load(file_path_):
    ...
    for ffi in range(len(file_path_)):
        ...
        if 0 == flags:
            ...
            mat_dir = os.path.join(os.path.dirname(file_path), "toothMat.txt")
            with open(mat_dir, 'r', encoding='utf-8') as f:
                for ann in f:
                    parts = ann.strip().split()   # split(' ') 대신 split() 권장 (연속 공백 안전)
                    if not parts:
                        continue

                    tttid = parts[0]
                    if tttid not in cfg.INDEX:
                        continue

                    index = int(cfg.INDEX[tttid]) - 1

                    nums = parts[1:]  # 숫자 토큰들만
                    if len(nums) == 9:
                        # toothID + 3x3 rotation
                        R = np.array(nums, dtype=np.float32).reshape(3, 3)

                    elif len(nums) == 16:
                        # toothID + 4x4 matrix (row-major)
                        M = np.array(nums, dtype=np.float32).reshape(4, 4)
                        R = M[:3, :3]

                    else:
                        # 포맷이 예상과 다르면 스킵(혹은 raise로 바꿔도 됨)
                        # print(f"[WARN] Unexpected format: tooth={tttid}, len(nums)={len(nums)} line={ann}")
                        continue
